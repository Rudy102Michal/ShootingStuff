...
func _physics_process(delta):
	...
	if players.size() == 1:
		...
		var min_dist : Vector3 = distances[0] if distances[0].length() < distances[1].length() else distances[1]
		var plane_distance : float = min_dist.project(global_transform.basis.z).length()
		distances[0] = distances[0].normalized() * min_dist.length()
		distances[1] = distances[1].normalized() * min_dist.length()
		...
		camera_distance = abs(projected_player1_pos.y - projected_player2_pos.y) / (2.0 * tan(deg2rad(fov / 2.0)))
		...
		cam_position = at_target + distance_translation * max(1.0, camera_distance)
